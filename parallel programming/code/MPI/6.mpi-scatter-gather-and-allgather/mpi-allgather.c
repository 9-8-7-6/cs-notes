...
// Run the command by
// mpicxx -std=c++11 mpi-allgather.c -o mpi-allgather
// mpirun -np 4 ./mpi-allgather 10
...
int main(int argc, char **argv) {
  ...
  float *sub_averages = (float *)malloc(sizeof(float) * world_size);
  assert(sub_averages != NULL);

  MPI_Allgather(&sub_average, 1, MPI_FLOAT, sub_averages, 1, MPI_FLOAT,
                MPI_COMM_WORLD);
